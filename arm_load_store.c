...
uint32_t get_address_h(arm_core p, uint32_t ins) {
  ...
  if ((bits_2724 == 1) && (bits_2221 == 2)) {
    ...
    return address;
  } else if ((bits_2724 == 1) && (bits_2221 == 0)) { // 2.Miscellaneous Loads and Stores - Register offset
    add_substract(u, & address, Rn_val(p, ins), Rm_val(p, ins));
    return address;
  } else if ((bits_2724 == 1) && (bits_2221 == 3)) { // 3.Miscellaneous Loads and Stores - Immediate pre-indexed
    uint32_t offset_8 = get_offset(ins);
    add_substract(u, & address, Rn_val(p, ins), offset_8);
    arm_write_register(p, (uint8_t) get_bits(ins, 19, 16), address);
    return address;
  } else if ((bits_2724 == 1) && (bits_2221 == 1)) { // 4.Miscellaneous Loads and Stores - Immediate pre-indexed
    add_substract(u, & address, Rn_val(p, ins), Rm_val(p, ins));
    arm_write_register(p, (uint8_t) get_bits(ins, 19, 16), address);
    return address;
  } else if ((bits_2724 == 0) && (bits_2221 == 2)) { // 5.Miscellaneous Loads and Stores - Immediate post-indexed
    address = Rn_val(p, ins);
    uint32_t offset_8 = get_offset(ins);
    condtion_pass_modify(p, ins, offset_8);
    return address;
  } else if ((bits_2724 == 0) && (bits_2221 == 0)) { // 6.Miscellaneous Loads and Stores - Register post-indexed
    address = Rn_val(p, ins);
    condtion_pass_modify(p, ins, Rm_val(p, ins));
    return address;
  } else
